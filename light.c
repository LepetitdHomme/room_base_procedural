...
// anti clockwise
triangle_t    corner_screen_triangle(state_t *state, coord_t ray) {
  triangle_t  triangle;


  if (state->grid[ray.x][ray.y] == CORNER_TOP_LEFT) {
  ...
  } else if (state->grid[ray.x][ray.y] == CORNER_TOP_RIGHT) {
  ...
  } else if (state->grid[ray.x][ray.y] == CORNER_BOT_LEFT) {
    // top left vertex of corner
  ...
  for (angle = LIGHT_ANGLE / 2 ; angle >= player_angle - LIGHT_ANGLE / 2 ; angle--) {
    ...
    for (distance = 0; distance < state->player->light ; distance++) {
      ...
      if (is_corner_wall(state->player->current_node->rect, coord_ray.x, coord_ray.y) == 0) {
        ...
        triangle_t corner_triangle = corner_screen_triangle(state, coord_ray);
        add_vertex(&vertices, &num_vertices, &capacity, corner_triangle.a.x, corner_triangle.a.y, opacity);
        add_vertex(&vertices, &num_vertices, &capacity, corner_triangle.b.x, corner_triangle.b.y, opacity);
        add_vertex(&vertices, &num_vertices, &capacity, corner_triangle.c.x, corner_triangle.c.y, opacity);
        ...
        // add_vertex(&vertices, &num_vertices, &capacity, coord_ray.x * state->tile_screen_size - state->scroll.x, coord_ray.y * state->tile_screen_size - state->scroll.y, 255);
        //break;
      //}
    }
  }
...
void          add_vertex(SDL_Vertex **vertices, size_t *num_vertices, size_t *capacity, int x, int y, int opacity) {
  ...
  (*vertices)[*num_vertices].color.r = r;
